<?php

namespace App\Http\Controllers\Frontend;

use Illuminate\Http\Request;
use App\Http\Controllers\Controller;
use App\Http\Controllers\Core\FrontendController;
use Illuminate\Support\Facades\Auth;
use Goutte\Client;
use Validator;
# Model
use Session;
use DB;
use App\Models\Apps;
use App\Models\MemberModel;
use App\User;
use App\Models\Testimonials;
use App\Models\News;

class ProfileController extends FrontendController
{
    public function __construct()
    {
        parent::__construct();
        
        if (Auth::check()) {
             $this->data['is_login'] = true;
        }
        $this->middleware('auth')->except(['index', 'uploadapp', 'filter']);
    }
    
    public function logout() {
    	if (Auth::check()) {
		    Auth::logout(); 
		}
		return redirect()->route('home');
    }
    
    public function showProfileForm() 
    {
    	$this->_DATA['user'] = User::find(Auth::id());
    	return view($this->_FOLDER_FRONTEND.'.profile.index',$this->_DATA);
    }
    
    public function uploadapp(Request $request) 
    {
    	if ($request['app_id']) {
    		$client = new Client();
	    	// Go to the symfony.com website
			$crawler = $client->request('GET', 'https://play.google.com/store/apps/details?id='.$request['app_id']);
			
			$this->_DATA['app_info'] = array(
										'app_id' => $request['app_id'],
										'app_title' => '',
										'app_hero_image' => '',
										'app_author_name' => '',
										'app_author_url' => '',
										'app_category_name' => '',
										'app_category_url' => '',
										'app_rating' => '',
										'app_rating_number' => '',
										'app_list_images' => array(),
										'app_description' => '',
										'app_email' => ''
										);
			$crawler->filter('h1.AHFaub')->each(function ($node) {
			    $this->_DATA['app_info']['app_title'] = $node->text();
			    return;
			});
			$crawler->filter('div.dQrBL img.ujDFqe')->each(function ($node) {
			    $this->_DATA['app_info']['app_hero_image'] = $node->attr('src');
			    return;
			});
			$crawler->filter('div.i4sPve a.hrTbp')->each(function ($node, $index) {
				if ($index == 0) {
					$this->_DATA['app_info']['app_author_name'] = $node->text();
					$this->_DATA['app_info']['app_author_url'] = $node->attr('href');
				} else if ($index == 1) {
					$this->_DATA['app_info']['app_category_name'] = $node->text();
					$this->_DATA['app_info']['app_category_url'] = $node->attr('href');
				} else {
					return;
				}
			});
			$crawler->filter('span.TBRnV')->each(function ($node) {
			    $this->_DATA['app_info']['app_rating_number'] = $node->text();
			    return;
			});
			$crawler->filter('div.dNLKff > c-wiz > div.pf5lIe > div')->each(function ($node) {
				$tmp = $node->attr('aria-label');
				$arr = explode(" ",$tmp);
				if (isset($arr[1]) && is_numeric($arr[1])) {
					$this->_DATA['app_info']['app_rating'] = $arr[1];
				}
			    return;
			});
			$crawler->filter('div.KDxLi img.lxGQyd')->each(function ($node) {
				$this->_DATA['app_info']['app_list_images'][] = $node->attr('src');
			});
			$crawler->filter('div.DWPxHb>content>div')->each(function ($node) {
			    $this->_DATA['app_info']['app_description'] = $node->html();
			    return;
			});
			
			// Check app_id exists before?
			$this->_DATA['app_info']['is_exists'] = "no";
			$this->_DATA['app_info']['is_owner'] = "no";
			$App_Check = Apps::where('app_id', $this->_DATA['app_info']['app_id'])->first();
			if ($App_Check) {
				$this->_DATA['app_info']['is_exists'] = "yes";
				if ($App_Check->member_id == Auth::id()) {
					$this->_DATA['app_info']['is_owner'] = "yes";
				}
			}
			// Check to save to database
			if ($request['save'] && $request['save'] == 'yes') {
		        $check = Validator::make($this->_DATA['app_info'], [
		            'app_title' => 'required',
		            'app_hero_image' => 'required',
		            'app_category_name' => 'required',
		            'app_category_url' => 'required',
		            'app_author_name' => 'required',
		            'app_author_url' => 'required'
		        ]);
		        if ($check->fails()) {
		            $this->_Data = array(
		                "type"   => "create",
		                "status" => "error"
		            );
		            return redirect()->back()->withErrors($check->errors())->with("messenger_show",$this->_Data["messenger_show"])->with("data",$this->_DATA['app_info']);
		        }else{
		            $columns = DB::connection()->getSchemaBuilder()->getColumnListing("apps");
		            
		            if ($this->_DATA['app_info']['is_exists'] == "no") {
		            	$App = new Apps();
		            	$App->member_id = Auth::id();
			            $App->created_at = date('Y-m-d H:i:s');
			            foreach ($this->_DATA['app_info'] as $key => $value) {
			                if ($value != null) {
			                    if (in_array($key,$columns)) {
			                    	if ($key == 'app_rating_number') {
			                    		$value = str_replace(',','',$value);
			                    	}
			                    	if (is_array($value)) {
			                    		$App->{$key} = implode(",",$value);
			                    	} else {
			                    		$App->{$key} = $this->clean_input($value);
			                    	}
			                    }
			                }
			            }
			            $App->save();
		            } else { // if ($this->_DATA['app_info']['is_owner'] == "yes") {
			            foreach ($this->_DATA['app_info'] as $key => $value) {
			                if ($value != null && $key != "app_id") {
			                    if (in_array($key,$columns)) {
			                    	if ($key == 'app_rating_number') {
			                    		$value = str_replace(',','',$value);
			                    	}
			                    	if (is_array($value)) {
			                    		$App_Check->{$key} = implode(",",$value);
			                    	} else {
			                    		$App_Check->{$key} = $this->clean_input($value);
			                    	}
			                    }
			                }
			            }
			            $App_Check->save();
		            }
		            Session::flash('message', 'This app has been saved successfully.');
		            return redirect()->route('uploadapp');
		        }
			}
    	}
    	
    	return view($this->_FOLDER_FRONTEND.'.profile.uploadapp',$this->_DATA);
    }
    
    public function update(Request $request){
        $check = Validator::make($request->all(), [
            'name' => 'required'
        ]);
        
        if ($check->fails()) {
            $this->_Data = array(
                "type"   => "create",
                "status" => "error",
                "post"   => @$request->input()
            );
            return redirect()->back()->withErrors($check->errors())->with("messenger_show",$this->_Data["messenger_show"])->with("user",$request->all());
        } else {
            $columns = DB::connection()->getSchemaBuilder()->getColumnListing("users");
            $input   = $request->input();
            $model   = User::find(Auth::id());
            foreach ($input as $key => $value) {
                if ($value != null && $key != "id") {
                    if (in_array($key,$columns)) {
                        $model->{$key} = $value;
                    }
                }
            }
            $model->save();
            Session::flash('message', 'Your profile has been updated successfully.');
		    return redirect()->route('profile.index');
        }
         
    }
    
    public function feelingForm(Request $request) {
    	// Get cmd
    	if ($request->input('cmd') != null) {
    		// Delete then remove and back
    		if (base64_decode($request->input('cmd')) == 'delete' && is_numeric(base64_decode($request->input('id')))) {
    			DB::table('testimonials')->where(['id' => base64_decode($request->input('id')), 'user_id' => Auth::id()])->delete();
    			Session::flash('message', 'This record has been deleted successful.');
		    	return redirect()->route('profile.feeling');
    		}
    		// Edit then show data on form
    		if (base64_decode($request->input('cmd')) == 'edit' && is_numeric(base64_decode($request->input('id')))) {
    			$this->_DATA['showform'] = true;
    			$this->_DATA['id'] = $request->input('id');
    			$this->_DATA['record'] = DB::table('testimonials')->where('id',base64_decode($request->input('id')))->where('user_id',Auth::id())->first();
    		}
    		// Add
    		if (base64_decode($request->input('cmd')) == 'add') {
    			$this->_DATA['id'] = 0;
    			$this->_DATA['showform'] = true;
    		}
    	}
    	
    	$this->_DATA['collections'] = DB::table('testimonials')->where('user_id', Auth::id())->orderBy('created_at', 'desc')->paginate(5);
    	
    	return view($this->_FOLDER_FRONTEND.'.profile.feeling',$this->_DATA);
    }
    
    public function feelingUpdate(Request $request){
        $check = Validator::make($request->all(), [
            'comments' => 'required'
        ]);
        
        if ($check->fails()) {
            $this->_Data = array(
                "type"   => "create",
                "status" => "error",
                "post"   => @$request->input()
            );
            return redirect()->back()->withErrors($check->errors())->with("messenger_show",$this->_Data["messenger_show"])->with("record",$request->all());
        } else {
            $columns = DB::connection()->getSchemaBuilder()->getColumnListing("testimonials");
            
            $input   = $request->input();
            $id = 0;
            if (is_numeric(base64_decode($request->input('id')))) {
            	$id = base64_decode($request->input('id'));
            }
        	if ($id == 0) {
        		// Add
        		$model = new Testimonials;
        		$model->user_id = Auth::id();
        	} else {
        		// Edit
        		$model = Testimonials::find($id);
        	}
        	foreach ($input as $key => $value) {
                if ($value != null && $key != "id") {
                    if (in_array($key,$columns)) {
                        $model->{$key} = $value;
                        
                    }
                }
            }
            $model->save();
            Session::flash('message', 'This record has been updated successfully.');
		    return redirect()->route('profile.feeling');
        }
         
    }
    
    
    public function newsForm(Request $request) {
    	// Get cmd
    	if ($request->input('cmd') != null) {
    		// Delete then remove and back
    		if (base64_decode($request->input('cmd')) == 'delete' && is_numeric(base64_decode($request->input('id')))) {
    			DB::table('news')->where(['id' => base64_decode($request->input('id')), 'user_id' => Auth::id()])->delete();
    			Session::flash('message', 'This record has been deleted successful.');
		    	return redirect()->route('profile.news');
    		}
    		// Edit then show data on form
    		if (base64_decode($request->input('cmd')) == 'edit' && is_numeric(base64_decode($request->input('id')))) {
    			$this->_DATA['showform'] = true;
    			$this->_DATA['id'] = $request->input('id');
    			$this->_DATA['record'] = DB::table('news')->where('id',base64_decode($request->input('id')))->where('user_id',Auth::id())->first();
    		}
    		// Add
    		if (base64_decode($request->input('cmd')) == 'add') {
    			$this->_DATA['id'] = 0;
    			$this->_DATA['showform'] = true;
    		}
    	}
    	
    	$this->_DATA['collections'] = DB::table('news')->where('user_id', Auth::id())->orderBy('created_at', 'desc')->paginate(5);
    	
    	return view($this->_FOLDER_FRONTEND.'.profile.news',$this->_DATA);
    }
    
    public function newsUpdate(Request $request){
        $check = Validator::make($request->all(), [
            'title' => 'required',
            'content' => 'required',
        ]);
        
        if ($check->fails()) {
            $this->_Data = array(
                "type"   => "create",
                "status" => "error",
                "post"   => @$request->input()
            );
            return redirect()->back()->withErrors($check->errors())->with("messenger_show",$this->_Data["messenger_show"])->with("record",$request->all());
        } else {
            $columns = DB::connection()->getSchemaBuilder()->getColumnListing("news");
            $input   = $request->input();
            
            $id = 0;
            if (is_numeric(base64_decode($request->input('id')))) {
            	$id = base64_decode($request->input('id'));
            }
        	if ($id == 0) {
        		// Add
        		$model = new News;
        		$model->user_id = Auth::id();
        	} else {
        		// Edit
        		$model = News::find($id);
        	}
        	/*
        	// Save summernote
            $content = $input['content'];
	        $dom = new \domdocument();
	        $dom->loadHtml($content, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
	        $images = $dom->getelementsbytagname('img');
	        foreach($images as $k => $img){
	            $data = $img->getattribute('src');
	            list($type, $data) = explode(';', $data);
	            list(, $data)      = explode(',', $data);
	            $data = base64_decode($data);
	            $image_name= time().$k.'.png';
	            $path = public_path() .'/uploads/news/'. $image_name;
	            file_put_contents($path, $data);
	            $img->removeattribute('src');
	            $img->setattribute('src', $image_name);
	        }
        	$model->content = $dom->savehtml();
        	*/
        	foreach ($input as $key => $value) {
                if ($value != null && $key != "id") {// && $key != "content"
                    if (in_array($key,$columns)) {
                        $model->{$key} = $value;
                    }
                }
            }
            $model->save();
            Session::flash('message', 'This record has been updated successfully.');
		    return redirect()->route('profile.news');
        }
         
    }
    
    private function clean_input($text) {
    	 $table = [
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xE2\x9D\xA4\xEF\xB8\x8F\xE2\x80\x8D\xF0\x9F\x92\x8B\xE2\x80\x8D\xF0\x9F\x91\xA8",
        "\xF0\x9F\x91\xA9\xE2\x80\x8D\xE2\x9D\xA4\xEF\xB8\x8F\xE2\x80\x8D\xF0\x9F\x92\x8B\xE2\x80\x8D\xF0\x9F\x91\xA9",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA6\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA7\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA7\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA7\xE2\x80\x8D\xF0\x9F\x91\xA7",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA6\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA7\xE2\x80\x8D\xF0\x9F\x91\xA7",
        "\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA6\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA7\xE2\x80\x8D\xF0\x9F\x91\xA7",
        "\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA7\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xE2\x9D\xA4\xEF\xB8\x8F\xE2\x80\x8D\xF0\x9F\x91\xA8",
        "\xF0\x9F\x91\xA9\xE2\x80\x8D\xE2\x9D\xA4\xEF\xB8\x8F\xE2\x80\x8D\xF0\x9F\x91\xA9",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA7",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA7",
        "\xF0\x9F\x91\xA8\xE2\x80\x8D\xF0\x9F\x91\xA9\xE2\x80\x8D\xF0\x9F\x91\xA7",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xA9",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xAF\xF0\x9F\x87\xB5",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xAF\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xAF\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xAD\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xAF\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xAE\xF0\x9F\x87\xB6",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xB5",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xB6",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xAD\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xAD\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xAD\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xAD\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xAD\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB5",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB6",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xBD",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xBB",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xA7",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xB5",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xB2\xF0\x9F\x87\xA9",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xBB",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xB1\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xB0\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xB6",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xAF",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xA9",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xBB",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xA9",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xA7",
        "\xF0\x9F\x87\xA7\xF0\x9F\x87\xA9",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xBD",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xA9\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xA9\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xAA\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xAB\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xA7",
        "\xF0\x9F\x87\xAB\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xAB\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xAB\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xAB\xF0\x9F\x87\xAF",
        "\xF0\x9F\x87\xAB\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xA9\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xA9\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xB5",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xA9\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xA9\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xA9\xF0\x9F\x87\xAF",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xBB",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xA8\xF0\x9F\x87\xBD",
        "\xF0\x9F\x87\xAC\xF0\x9F\x87\xA9",
        "\xF0\x9F\x87\xA6\xF0\x9F\x87\xB6",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xBA\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xBA\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xA7",
        "\xF0\x9F\x87\xBA\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xBA\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xBA\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xA9",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xBA\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xBB\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xBB\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xB7\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xBC\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xB7\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xBC\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xB7\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xBB\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xBB\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xBB\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xBB\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xBB\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xB7\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xBB",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xBD",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xA8",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xAF",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xA9",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xBB",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xAF",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xB8\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xB9\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xB6\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xB7\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xAD",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xAE",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xBF",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xB4",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xAC",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xB5",
        "\xF0\x9F\x87\xBD\xF0\x9F\x87\xB0",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xBA",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xB1",
        "\xF0\x9F\x87\xB4\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xB8",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xB7",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xB3",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xBE\xF0\x9F\x87\xB9",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xBE",
        "\xF0\x9F\x87\xBE\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xB5\xF0\x9F\x87\xB2",
        "\xF0\x9F\x87\xBF\xF0\x9F\x87\xA6",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xAB",
        "\xF0\x9F\x87\xB3\xF0\x9F\x87\xAA",
        "\xF0\x9F\x87\xBF\xF0\x9F\x87\xBC",
        "\xF0\x9F\x87\xBF\xF0\x9F\x87\xB2",
        "\xF0\x9F\x85\xBE\xEF\xB8\x8F",
        "\xF0\x9F\x85\xBF\xEF\xB8\x8F",
        "\x37\xEF\xB8\x8F\xE2\x83\xA3",
        "\x36\xEF\xB8\x8F\xE2\x83\xA3",
        "\xF0\x9F\x88\x82\xEF\xB8\x8F",
        "\x35\xEF\xB8\x8F\xE2\x83\xA3",
        "\x39\xEF\xB8\x8F\xE2\x83\xA3",
        "\x38\xEF\xB8\x8F\xE2\x83\xA3",
        "\xF0\x9F\x88\xAF\xEF\xB8\x8F",
        "\x32\xEF\xB8\x8F\xE2\x83\xA3",
        "\xF0\x9F\x88\x9A\xEF\xB8\x8F",
        "\x31\xEF\xB8\x8F\xE2\x83\xA3",
        "\x30\xEF\xB8\x8F\xE2\x83\xA3",
        "\x23\xEF\xB8\x8F\xE2\x83\xA3",
        "\x34\xEF\xB8\x8F\xE2\x83\xA3",
        "\x33\xEF\xB8\x8F\xE2\x83\xA3",
        "\xF0\x9F\x85\xB1\xEF\xB8\x8F",
        "\xF0\x9F\x85\xB0\xEF\xB8\x8F",
        "\xF0\x9F\x88\xB7\xEF\xB8\x8F",
        "\xF0\x9F\x80\x84\xEF\xB8\x8F",
        "\xE2\x98\x95\xEF\xB8\x8F",
        "\xE2\x9B\xB2\xEF\xB8\x8F",
        "\xE2\x98\x84\xEF\xB8\x8F",
        "\xE2\x98\x8E\xEF\xB8\x8F",
        "\xE2\x98\x94\xEF\xB8\x8F",
        "\xE2\x98\x91\xEF\xB8\x8F",
        "\xE2\x84\xB9\xEF\xB8\x8F",
        "\xE2\x97\x80\xEF\xB8\x8F",
        "\xE2\x96\xB6\xEF\xB8\x8F",
        "\xE2\x86\xA9\xEF\xB8\x8F",
        "\xE2\x86\x99\xEF\xB8\x8F",
        "\xE2\x97\xBB\xEF\xB8\x8F",
        "\xE2\x86\xAA\xEF\xB8\x8F",
        "\xE2\x8C\x9A\xEF\xB8\x8F",
        "\xE2\x8C\xA8\xEF\xB8\x8F",
        "\xE2\x93\x82\xEF\xB8\x8F",
        "\xE2\x96\xAA\xEF\xB8\x8F",
        "\xE2\x8C\x9B\xEF\xB8\x8F",
        "\xE2\x96\xAB\xEF\xB8\x8F",
        "\xE2\x86\x98\xEF\xB8\x8F",
        "\xE2\x97\xBC\xEF\xB8\x8F",
        "\xE2\x81\x89\xEF\xB8\x8F",
        "\xE2\x84\xA2\xEF\xB8\x8F",
        "\xE2\x80\xBC\xEF\xB8\x8F",
        "\xE2\x98\x81\xEF\xB8\x8F",
        "\xE2\x98\x82\xEF\xB8\x8F",
        "\xE2\x98\x80\xEF\xB8\x8F",
        "\xE2\x86\x94\xEF\xB8\x8F",
        "\xE2\x97\xBD\xEF\xB8\x8F",
        "\xE2\x86\x96\xEF\xB8\x8F",
        "\xE2\x86\x95\xEF\xB8\x8F",
        "\xE2\x97\xBE\xEF\xB8\x8F",
        "\xE2\x98\x83\xEF\xB8\x8F",
        "\xE2\x86\x97\xEF\xB8\x8F",
        "\xE2\x9C\xB4\xEF\xB8\x8F",
        "\xE2\x9D\x84\xEF\xB8\x8F",
        "\xE2\x9A\xA1\xEF\xB8\x8F",
        "\xE2\x9A\xAA\xEF\xB8\x8F",
        "\xE2\x9C\xB3\xEF\xB8\x8F",
        "\xE2\x9C\x9D\xEF\xB8\x8F",
        "\xE2\x9C\xA1\xEF\xB8\x8F",
        "\xE2\x9A\xAB\xEF\xB8\x8F",
        "\xE2\x9D\x87\xEF\xB8\x8F",
        "\xE2\x9A\xA0\xEF\xB8\x8F",
        "\xE2\x99\xBF\xEF\xB8\x8F",
        "\xE2\x99\xBB\xEF\xB8\x8F",
        "\xE2\xA4\xB4\xEF\xB8\x8F",
        "\xE2\x9E\xA1\xEF\xB8\x8F",
        "\xE2\x9A\x93\xEF\xB8\x8F",
        "\xE2\x9D\x97\xEF\xB8\x8F",
        "\xE2\x9D\xA3\xEF\xB8\x8F",
        "\xE2\x9D\xA4\xEF\xB8\x8F",
        "\xE2\x9C\x96\xEF\xB8\x8F",
        "\xE2\x9C\x94\xEF\xB8\x8F",
        "\xE2\x9B\xBD\xEF\xB8\x8F",
        "\xE2\x9C\x82\xEF\xB8\x8F",
        "\xE2\x9C\x88\xEF\xB8\x8F",
        "\xE2\x9B\xBA\xEF\xB8\x8F",
        "\xE2\x9B\xB5\xEF\xB8\x8F",
        "\xE2\x9B\xB3\xEF\xB8\x8F",
        "\xE2\x98\x9D\xEF\xB8\x8F",
        "\xE2\x9B\xAA\xEF\xB8\x8F",
        "\xE2\x9C\x89\xEF\xB8\x8F",
        "\xE2\x9B\x85\xEF\xB8\x8F",
        "\xE2\x9C\x8F\xEF\xB8\x8F",
        "\xE2\x9C\x92\xEF\xB8\x8F",
        "\xE2\x9A\xBD\xEF\xB8\x8F",
        "\xE2\x9A\xBE\xEF\xB8\x8F",
        "\xE2\x9C\x8D\xEF\xB8\x8F",
        "\xE2\x9B\x84\xEF\xB8\x8F",
        "\xE2\x9C\x8C\xEF\xB8\x8F",
        "\xE2\xA4\xB5\xEF\xB8\x8F",
        "\xE2\x9B\x94\xEF\xB8\x8F",
        "\xE2\x99\x8A\xEF\xB8\x8F",
        "\xE2\x99\x89\xEF\xB8\x8F",
        "\xE2\x99\x88\xEF\xB8\x8F",
        "\xE2\x99\x8B\xEF\xB8\x8F",
        "\xE2\x99\x8C\xEF\xB8\x8F",
        "\xE2\x99\x8F\xEF\xB8\x8F",
        "\xE2\x99\x8E\xEF\xB8\x8F",
        "\xE2\x99\x8D\xEF\xB8\x8F",
        "\xE2\x98\xBA\xEF\xB8\x8F",
        "\xE2\x98\xB9\xEF\xB8\x8F",
        "\xE2\x98\xA0\xEF\xB8\x8F",
        "\xE2\x99\xA8\xEF\xB8\x8F",
        "\xE2\x98\xA2\xEF\xB8\x8F",
        "\xE2\x98\xA3\xEF\xB8\x8F",
        "\xE2\x98\xA6\xEF\xB8\x8F",
        "\xE2\x98\xB8\xEF\xB8\x8F",
        "\xE2\x98\xAF\xEF\xB8\x8F",
        "\xE2\x98\xAA\xEF\xB8\x8F",
        "\xE2\x99\x90\xEF\xB8\x8F",
        "\xE2\x98\xAE\xEF\xB8\x8F",
        "\xE2\x99\xA6\xEF\xB8\x8F",
        "\xE2\xAD\x90\xEF\xB8\x8F",
        "\xE2\x99\xA3\xEF\xB8\x8F",
        "\xE2\xAC\x86\xEF\xB8\x8F",
        "\xE2\xAC\x9C\xEF\xB8\x8F",
        "\xE2\xAC\x85\xEF\xB8\x8F",
        "\xE2\xAC\x9B\xEF\xB8\x8F",
        "\xE2\xAC\x87\xEF\xB8\x8F",
        "\xE2\x99\xA5\xEF\xB8\x8F",
        "\xE3\x80\xB0\xEF\xB8\x8F",
        "\xE2\xAD\x95\xEF\xB8\x8F",
        "\xE3\x8A\x97\xEF\xB8\x8F",
        "\xE3\x8A\x99\xEF\xB8\x8F",
        "\xE2\x99\x92\xEF\xB8\x8F",
        "\xE2\x99\x91\xEF\xB8\x8F",
        "\xE3\x80\xBD\xEF\xB8\x8F",
        "\xE2\x99\x93\xEF\xB8\x8F",
        "\xE2\x99\xA0\xEF\xB8\x8F",
        "\xC2\xA9\xEF\xB8\x8F",
        "\xC2\xAE\xEF\xB8\x8F",
        "\xF0\x9F\x95\x9F",
        "\xF0\x9F\x95\x9A",
        "\xF0\x9F\x95\x98",
        "\xF0\x9F\x95\x9E",
        "\xF0\x9F\x95\x9D",
        "\xF0\x9F\x95\x99",
        "\xF0\x9F\x95\x9C",
        "\xF0\x9F\x95\x9B",
        "\xF0\x9F\x95\xA5",
        "\xF0\x9F\x95\xAF",
        "\xF0\x9F\x95\xA7",
        "\xF0\x9F\x95\xB0",
        "\xF0\x9F\x95\xB3",
        "\xF0\x9F\x95\xB4",
        "\xF0\x9F\x95\xA6",
        "\xF0\x9F\x95\x97",
        "\xF0\x9F\x95\xA1",
        "\xF0\x9F\x95\xA2",
        "\xF0\x9F\x95\xA3",
        "\xF0\x9F\x95\xA4",
        "\xF0\x9F\x95\xA0",
        "\xF0\x9F\x95\x8D",
        "\xF0\x9F\x94\xBC",
        "\xF0\x9F\x95\xB5",
        "\xF0\x9F\x94\xBD",
        "\xF0\x9F\x95\x89",
        "\xF0\x9F\x94\xBB",
        "\xF0\x9F\x94\xBA",
        "\xF0\x9F\x94\xB5",
        "\xF0\x9F\x94\xB7",
        "\xF0\x9F\x94\xB8",
        "\xF0\x9F\x94\xB9",
        "\xF0\x9F\x95\x8A",
        "\xF0\x9F\x95\x8B",
        "\xF0\x9F\x95\x92",
        "\xF0\x9F\x95\x93",
        "\xF0\x9F\x95\x94",
        "\xF0\x9F\x95\x95",
        "\xF0\x9F\x95\x91",
        "\xF0\x9F\x95\x90",
        "\xF0\x9F\x95\x8C",
        "\xF0\x9F\x94\xB6",
        "\xF0\x9F\x95\x8E",
        "\xF0\x9F\x95\x96",
        "\xF0\x9F\x97\x83",
        "\xF0\x9F\x97\xBA",
        "\xF0\x9F\x97\xBB",
        "\xF0\x9F\x97\xBC",
        "\xF0\x9F\x97\xBD",
        "\xF0\x9F\x97\xB3",
        "\xF0\x9F\x97\xAF",
        "\xF0\x9F\x97\x9E",
        "\xF0\x9F\x97\xA1",
        "\xF0\x9F\x97\xA3",
        "\xF0\x9F\x97\xA8",
        "\xF0\x9F\x97\xBE",
        "\xF0\x9F\x94\xB4",
        "\xF0\x9F\x98\x85",
        "\xF0\x9F\x98\x86",
        "\xF0\x9F\x98\x87",
        "\xF0\x9F\x98\x88",
        "\xF0\x9F\x98\x84",
        "\xF0\x9F\x98\x83",
        "\xF0\x9F\x97\xBF",
        "\xF0\x9F\x98\x80",
        "\xF0\x9F\x98\x81",
        "\xF0\x9F\x98\x82",
        "\xF0\x9F\x97\x9D",
        "\xF0\x9F\x97\x9C",
        "\xF0\x9F\x96\x8C",
        "\xF0\x9F\x96\x8D",
        "\xF0\x9F\x96\x90",
        "\xF0\x9F\x96\x95",
        "\xF0\x9F\x96\x8B",
        "\xF0\x9F\x96\x8A",
        "\xF0\x9F\x95\xB7",
        "\xF0\x9F\x95\xB8",
        "\xF0\x9F\x95\xB9",
        "\xF0\x9F\x96\x87",
        "\xF0\x9F\x96\x96",
        "\xF0\x9F\x96\xA5",
        "\xF0\x9F\x97\x84",
        "\xF0\x9F\x97\x91",
        "\xF0\x9F\x97\x92",
        "\xF0\x9F\x97\x93",
        "\xF0\x9F\x97\x82",
        "\xF0\x9F\x96\xBC",
        "\xF0\x9F\x96\xA8",
        "\xF0\x9F\x96\xB1",
        "\xF0\x9F\x96\xB2",
        "\xF0\x9F\x95\xB6",
        "\xF0\x9F\x94\x8D",
        "\xF0\x9F\x93\xB3",
        "\xF0\x9F\x93\xB4",
        "\xF0\x9F\x93\xB5",
        "\xF0\x9F\x93\xB6",
        "\xF0\x9F\x93\xB2",
        "\xF0\x9F\x93\xB1",
        "\xF0\x9F\x93\xAD",
        "\xF0\x9F\x93\xAE",
        "\xF0\x9F\x93\xAF",
        "\xF0\x9F\x93\xB0",
        "\xF0\x9F\x93\xB7",
        "\xF0\x9F\x93\xB8",
        "\xF0\x9F\x94\x80",
        "\xF0\x9F\x94\x81",
        "\xF0\x9F\x94\x82",
        "\xF0\x9F\x94\x83",
        "\xF0\x9F\x93\xBF",
        "\xF0\x9F\x93\xBD",
        "\xF0\x9F\x93\xB9",
        "\xF0\x9F\x93\xBA",
        "\xF0\x9F\x93\xBB",
        "\xF0\x9F\x93\xBC",
        "\xF0\x9F\x93\xAC",
        "\xF0\x9F\x93\xAB",
        "\xF0\x9F\x93\x9C",
        "\xF0\x9F\x93\x9D",
        "\xF0\x9F\x93\x9E",
        "\xF0\x9F\x93\x9F",
        "\xF0\x9F\x93\x9B",
        "\xF0\x9F\x93\x9A",
        "\xF0\x9F\x93\x97",
        "\xF0\x9F\x98\x89",
        "\xF0\x9F\x93\x98",
        "\xF0\x9F\x93\x99",
        "\xF0\x9F\x93\xA0",
        "\xF0\x9F\x93\xA1",
        "\xF0\x9F\x93\xA7",
        "\xF0\x9F\x93\xA8",
        "\xF0\x9F\x93\xA9",
        "\xF0\x9F\x93\xAA",
        "\xF0\x9F\x93\xA6",
        "\xF0\x9F\x93\xA5",
        "\xF0\x9F\x93\xA2",
        "\xF0\x9F\x93\xA3",
        "\xF0\x9F\x93\xA4",
        "\xF0\x9F\x94\x84",
        "\xF0\x9F\x94\x85",
        "\xF0\x9F\x94\xA4",
        "\xF0\x9F\x94\xA5",
        "\xF0\x9F\x94\xA6",
        "\xF0\x9F\x94\xA7",
        "\xF0\x9F\x94\xA3",
        "\xF0\x9F\x94\xA2",
        "\xF0\x9F\x94\x9E",
        "\xF0\x9F\x94\x9F",
        "\xF0\x9F\x94\xA0",
        "\xF0\x9F\x94\xA1",
        "\xF0\x9F\x94\xA8",
        "\xF0\x9F\x94\xA9",
        "\xF0\x9F\x94\xAF",
        "\xF0\x9F\x94\xB0",
        "\xF0\x9F\x94\xB1",
        "\xF0\x9F\x94\xB2",
        "\xF0\x9F\x94\xAE",
        "\xF0\x9F\x94\xAD",
        "\xF0\x9F\x94\xAA",
        "\xF0\x9F\x94\xAB",
        "\xF0\x9F\x94\xAC",
        "\xF0\x9F\x94\x9D",
        "\xF0\x9F\x94\x9C",
        "\xF0\x9F\x94\x8C",
        "\xF0\x9F\x94\x8E",
        "\xF0\x9F\x94\x8F",
        "\xF0\x9F\x94\x90",
        "\xF0\x9F\x94\x8B",
        "\xF0\x9F\x94\x8A",
        "\xF0\x9F\x94\x86",
        "\xF0\x9F\x94\x87",
        "\xF0\x9F\x94\x88",
        "\xF0\x9F\x94\x89",
        "\xF0\x9F\x94\x91",
        "\xF0\x9F\x94\x92",
        "\xF0\x9F\x94\x98",
        "\xF0\x9F\x94\x99",
        "\xF0\x9F\x94\x9A",
        "\xF0\x9F\x94\x9B",
        "\xF0\x9F\x94\x97",
        "\xF0\x9F\x94\x96",
        "\xF0\x9F\x94\x93",
        "\xF0\x9F\x94\x94",
        "\xF0\x9F\x94\x95",
        "\xF0\x9F\x94\xB3",
        "\xF0\x9F\x9A\xA9",
        "\xF0\x9F\x9A\xB5",
        "\xF0\x9F\x9A\xB6",
        "\xF0\x9F\x9A\xB7",
        "\xF0\x9F\x9A\xB8",
        "\xF0\x9F\x9A\xB4",
        "\xF0\x9F\x9A\xB3",
        "\xF0\x9F\x9A\xAF",
        "\xF0\x9F\x9A\xB0",
        "\xF0\x9F\x9A\xB1",
        "\xF0\x9F\x9A\xB2",
        "\xF0\x9F\x9A\xB9",
        "\xF0\x9F\x9A\xBA",
        "\xF0\x9F\x9B\x81",
        "\xF0\x9F\x9B\x82",
        "\xF0\x9F\x9B\x83",
        "\xF0\x9F\x9B\x84",
        "\xF0\x9F\x9B\x80",
        "\xF0\x9F\x9A\xBF",
        "\xF0\x9F\x9A\xBB",
        "\xF0\x9F\x9A\xBC",
        "\xF0\x9F\x9A\xBD",
        "\xF0\x9F\x9A\xBE",
        "\xF0\x9F\x9A\xAE",
        "\xF0\x9F\x9A\xAD",
        "\xF0\x9F\x9A\x9D",
        "\xF0\x9F\x9A\x9E",
        "\xF0\x9F\x9A\x9F",
        "\xF0\x9F\x9A\xA0",
        "\xF0\x9F\x9A\x9C",
        "\xF0\x9F\x9A\x9B",
        "\xF0\x9F\x9A\x97",
        "\xF0\x9F\x9A\x98",
        "\xF0\x9F\x9A\x99",
        "\xF0\x9F\x9A\x9A",
        "\xF0\x9F\x9A\xA1",
        "\xF0\x9F\x9A\xA2",
        "\xF0\x9F\x9A\xA8",
        "\xF0\x9F\x9A\xAA",
        "\xF0\x9F\x9A\xAB",
        "\xF0\x9F\x9A\xAC",
        "\xF0\x9F\x9A\xA7",
        "\xF0\x9F\x9A\xA6",
        "\xF0\x9F\x9A\xA3",
        "\xF0\x9F\x9A\xA4",
        "\xF0\x9F\x9A\xA5",
        "\xF0\x9F\x9B\x85",
        "\xF0\x9F\x9B\x8B",
        "\xF0\x9F\xA6\x84",
        "\xF0\x9F\xA7\x80",
        "\x23\xE2\x83\xA3",
        "\x2A\xE2\x83\xA3",
        "\xF0\x9F\xA6\x83",
        "\xF0\x9F\xA6\x82",
        "\xF0\x9F\xA4\x97",
        "\xF0\x9F\xA4\x98",
        "\xF0\x9F\xA6\x80",
        "\xF0\x9F\xA6\x81",
        "\x30\xE2\x83\xA3",
        "\x31\xE2\x83\xA3",
        "\x37\xE2\x83\xA3",
        "\x38\xE2\x83\xA3",
        "\x39\xE2\x83\xA3",
        "\xF0\x9F\x93\x96",
        "\x36\xE2\x83\xA3",
        "\x35\xE2\x83\xA3",
        "\x32\xE2\x83\xA3",
        "\x33\xE2\x83\xA3",
        "\x34\xE2\x83\xA3",
        "\xF0\x9F\xA4\x96",
        "\xF0\x9F\xA4\x95",
        "\xF0\x9F\x9B\xA1",
        "\xF0\x9F\x9B\xA2",
        "\xF0\x9F\x9B\xA3",
        "\xF0\x9F\x9B\xA4",
        "\xF0\x9F\x9B\xA0",
        "\xF0\x9F\x9B\x90",
        "\xF0\x9F\x9B\x8C",
        "\xF0\x9F\x9B\x8D",
        "\xF0\x9F\x9B\x8E",
        "\xF0\x9F\x9B\x8F",
        "\xF0\x9F\x9B\xA5",
        "\xF0\x9F\x9B\xA9",
        "\xF0\x9F\xA4\x91",
        "\xF0\x9F\xA4\x92",
        "\xF0\x9F\xA4\x93",
        "\xF0\x9F\xA4\x94",
        "\xF0\x9F\xA4\x90",
        "\xF0\x9F\x9B\xB3",
        "\xF0\x9F\x9B\xAB",
        "\xF0\x9F\x9B\xAC",
        "\xF0\x9F\x9B\xB0",
        "\xF0\x9F\x9A\x96",
        "\xF0\x9F\x9A\x95",
        "\xF0\x9F\x98\xA8",
        "\xF0\x9F\x98\xA9",
        "\xF0\x9F\x98\xAA",
        "\xF0\x9F\x98\xAB",
        "\xF0\x9F\x98\xA7",
        "\xF0\x9F\x98\xA6",
        "\xF0\x9F\x98\xA2",
        "\xF0\x9F\x98\xA3",
        "\xF0\x9F\x98\xA4",
        "\xF0\x9F\x98\xA5",
        "\xF0\x9F\x98\xAC",
        "\xF0\x9F\x98\xAD",
        "\xF0\x9F\x98\xB3",
        "\xF0\x9F\x98\xB4",
        "\xF0\x9F\x98\xB5",
        "\xF0\x9F\x98\xB6",
        "\xF0\x9F\x98\xB2",
        "\xF0\x9F\x98\xB1",
        "\xF0\x9F\x98\xAE",
        "\xF0\x9F\x98\xAF",
        "\xF0\x9F\x98\xB0",
        "\xF0\x9F\x98\xA1",
        "\xF0\x9F\x98\xA0",
        "\xF0\x9F\x98\x91",
        "\xF0\x9F\x98\x92",
        "\xF0\x9F\x98\x93",
        "\xF0\x9F\x98\x94",
        "\xF0\x9F\x98\x90",
        "\xF0\x9F\x98\x8F",
        "\xF0\x9F\x98\x8B",
        "\xF0\x9F\x98\x8C",
        "\xF0\x9F\x98\x8D",
        "\xF0\x9F\x98\x8E",
        "\xF0\x9F\x98\x95",
        "\xF0\x9F\x98\x96",
        "\xF0\x9F\x98\x9C",
        "\xF0\x9F\x98\x9D",
        "\xF0\x9F\x98\x9E",
        "\xF0\x9F\x98\x9F",
        "\xF0\x9F\x98\x9B",
        "\xF0\x9F\x98\x9A",
        "\xF0\x9F\x98\x97",
        "\xF0\x9F\x98\x98",
        "\xF0\x9F\x98\x99",
        "\xF0\x9F\x98\xB7",
        "\xF0\x9F\x98\xB8",
        "\xF0\x9F\x9A\x86",
        "\xF0\x9F\x9A\x87",
        "\xF0\x9F\x9A\x88",
        "\xF0\x9F\x9A\x89",
        "\xF0\x9F\x9A\x85",
        "\xF0\x9F\x9A\x84",
        "\xF0\x9F\x9A\x80",
        "\xF0\x9F\x9A\x81",
        "\xF0\x9F\x9A\x82",
        "\xF0\x9F\x9A\x83",
        "\xF0\x9F\x9A\x8A",
        "\xF0\x9F\x9A\x8B",
        "\xF0\x9F\x9A\x91",
        "\xF0\x9F\x9A\x92",
        "\xF0\x9F\x9A\x93",
        "\xF0\x9F\x9A\x94",
        "\xF0\x9F\x9A\x90",
        "\xF0\x9F\x9A\x8F",
        "\xF0\x9F\x9A\x8C",
        "\xF0\x9F\x9A\x8D",
        "\xF0\x9F\x9A\x8E",
        "\xF0\x9F\x99\x8F",
        "\xF0\x9F\x99\x8E",
        "\xF0\x9F\x98\xBF",
        "\xF0\x9F\x99\x80",
        "\xF0\x9F\x99\x81",
        "\xF0\x9F\x99\x82",
        "\xF0\x9F\x98\xBE",
        "\xF0\x9F\x98\xBD",
        "\xF0\x9F\x98\xB9",
        "\xF0\x9F\x98\xBA",
        "\xF0\x9F\x98\xBB",
        "\xF0\x9F\x98\xBC",
        "\xF0\x9F\x99\x83",
        "\xF0\x9F\x99\x84",
        "\xF0\x9F\x99\x8A",
        "\xF0\x9F\x99\x8B",
        "\xF0\x9F\x99\x8C",
        "\xF0\x9F\x99\x8D",
        "\xF0\x9F\x99\x89",
        "\xF0\x9F\x99\x88",
        "\xF0\x9F\x99\x85",
        "\xF0\x9F\x99\x86",
        "\xF0\x9F\x99\x87",
        "\xF0\x9F\x98\x8A",
        "\xF0\x9F\x92\x97",
        "\xF0\x9F\x8E\x85",
        "\xF0\x9F\x8E\x84",
        "\xF0\x9F\x93\x95",
        "\xF0\x9F\x8E\x87",
        "\xF0\x9F\x8E\x89",
        "\xF0\x9F\x8E\x88",
        "\xF0\x9F\x8E\x83",
        "\xF0\x9F\x8E\x82",
        "\xF0\x9F\x8D\xBD",
        "\xF0\x9F\x8D\xBC",
        "\xF0\x9F\x8D\xBE",
        "\xF0\x9F\x8D\xBF",
        "\xF0\x9F\x8E\x81",
        "\xF0\x9F\x8E\x80",
        "\xF0\x9F\x8E\x8A",
        "\xF0\x9F\x8E\x8B",
        "\xF0\x9F\x8E\x96",
        "\xF0\x9F\x8E\x93",
        "\xF0\x9F\x8E\x97",
        "\xF0\x9F\x8E\x99",
        "\xF0\x9F\x8E\x9B",
        "\xF0\x9F\x8E\x9A",
        "\xF0\x9F\x8E\x92",
        "\xF0\x9F\x8E\x91",
        "\xF0\x9F\x8E\x8D",
        "\xF0\x9F\x8E\x8C",
        "\xF0\x9F\x8E\x8E",
        "\xF0\x9F\x8E\x8F",
        "\xF0\x9F\x8E\x90",
        "\xF0\x9F\x8D\xBB",
        "\xF0\x9F\x8D\xBA",
        "\xF0\x9F\x8D\xA6",
        "\xF0\x9F\x8D\xA5",
        "\xF0\x9F\x8D\xA7",
        "\xF0\x9F\x8D\xA8",
        "\xF0\x9F\x8D\xAA",
        "\xF0\x9F\x8D\xA9",
        "\xF0\x9F\x8D\xA4",
        "\xF0\x9F\x8D\xA3",
        "\xF0\x9F\x8D\x9E",
        "\xF0\x9F\x8D\x9D",
        "\xF0\x9F\x8D\x9F",
        "\xF0\x9F\x8D\xA0",
        "\xF0\x9F\x8D\xA2",
        "\xF0\x9F\x8D\xA1",
        "\xF0\x9F\x8D\xAB",
        "\xF0\x9F\x8D\xAC",
        "\xF0\x9F\x8D\xB5",
        "\xF0\x9F\x8D\xB4",
        "\xF0\x9F\x8D\xB6",
        "\xF0\x9F\x8D\xB7",
        "\xF0\x9F\x8D\xB9",
        "\xF0\x9F\x8D\xB8",
        "\xF0\x9F\x8D\xB3",
        "\xF0\x9F\x8D\xB2",
        "\xF0\x9F\x8D\xAE",
        "\xF0\x9F\x8D\xAD",
        "\xF0\x9F\x8D\xAF",
        "\xF0\x9F\x8D\xB0",
        "\xF0\x9F\x8D\xB1",
        "\xF0\x9F\x8E\x9E",
        "\xF0\x9F\x8E\x9F",
        "\xF0\x9F\x8F\x88",
        "\xF0\x9F\x8F\x87",
        "\xF0\x9F\x8F\x89",
        "\xF0\x9F\x8F\x8A",
        "\xF0\x9F\x8F\x8C",
        "\xF0\x9F\x8F\x8B",
        "\xF0\x9F\x8F\x86",
        "\xF0\x9F\x8F\x85",
        "\xF0\x9F\x8F\x80",
        "\xF0\x9F\x8E\xBF",
        "\xF0\x9F\x8F\x81",
        "\xF0\x9F\x8F\x82",
        "\xF0\x9F\x8F\x84",
        "\xF0\x9F\x8F\x83",
        "\xF0\x9F\x8F\x8D",
        "\xF0\x9F\x8F\x8E",
        "\xF0\x9F\x8F\x97",
        "\xF0\x9F\x8F\x96",
        "\xF0\x9F\x8F\x98",
        "\xF0\x9F\x8F\x99",
        "\xF0\x9F\x8F\x9B",
        "\xF0\x9F\x8F\x9A",
        "\xF0\x9F\x8F\x95",
        "\xF0\x9F\x8F\x94",
        "\xF0\x9F\x8F\x90",
        "\xF0\x9F\x8F\x8F",
        "\xF0\x9F\x8F\x91",
        "\xF0\x9F\x8F\x92",
        "\xF0\x9F\x8F\x93",
        "\xF0\x9F\x8E\xBE",
        "\xF0\x9F\x8E\xBD",
        "\xF0\x9F\x8E\xA9",
        "\xF0\x9F\x8E\xA8",
        "\xF0\x9F\x8E\xAA",
        "\xF0\x9F\x8E\xAB",
        "\xF0\x9F\x8E\xAD",
        "\xF0\x9F\x8E\xAC",
        "\xF0\x9F\x8E\xA7",
        "\xF0\x9F\x8E\xA6",
        "\xF0\x9F\x8E\xA1",
        "\xF0\x9F\x8E\xA0",
        "\xF0\x9F\x8E\xA2",
        "\xF0\x9F\x8E\xA3",
        "\xF0\x9F\x8E\xA5",
        "\xF0\x9F\x8E\xA4",
        "\xF0\x9F\x8E\xAE",
        "\xF0\x9F\x8E\xAF",
        "\xF0\x9F\x8E\xB8",
        "\xF0\x9F\x8E\xB7",
        "\xF0\x9F\x8E\xB9",
        "\xF0\x9F\x8E\xBA",
        "\xF0\x9F\x8E\xBC",
        "\xF0\x9F\x8E\xBB",
        "\xF0\x9F\x8E\xB6",
        "\xF0\x9F\x8E\xB5",
        "\xF0\x9F\x8E\xB1",
        "\xF0\x9F\x8E\xB0",
        "\xF0\x9F\x8E\xB2",
        "\xF0\x9F\x8E\xB3",
        "\xF0\x9F\x8E\xB4",
        "\xF0\x9F\x8D\x9C",
        "\xF0\x9F\x8D\x9B",
        "\xF0\x9F\x8C\x88",
        "\xF0\x9F\x8C\x87",
        "\xF0\x9F\x8C\x89",
        "\xF0\x9F\x8C\x8A",
        "\xF0\x9F\x8C\x8C",
        "\xF0\x9F\x8C\x8B",
        "\xF0\x9F\x8C\x86",
        "\xF0\x9F\x8C\x85",
        "\xF0\x9F\x8C\x80",
        "\xF0\x9F\x89\x91",
        "\xF0\x9F\x8C\x81",
        "\xF0\x9F\x8C\x82",
        "\xF0\x9F\x8C\x84",
        "\xF0\x9F\x8C\x83",
        "\xF0\x9F\x8C\x8D",
        "\xF0\x9F\x8C\x8E",
        "\xF0\x9F\x8C\x97",
        "\xF0\x9F\x8C\x96",
        "\xF0\x9F\x8C\x98",
        "\xF0\x9F\x8C\x99",
        "\xF0\x9F\x8C\x9B",
        "\xF0\x9F\x8C\x9A",
        "\xF0\x9F\x8C\x95",
        "\xF0\x9F\x8C\x94",
        "\xF0\x9F\x8C\x90",
        "\xF0\x9F\x8C\x8F",
        "\xF0\x9F\x8C\x91",
        "\xF0\x9F\x8C\x92",
        "\xF0\x9F\x8C\x93",
        "\xF0\x9F\x89\x90",
        "\xF0\x9F\x88\xBA",
        "\xF0\x9F\x86\x93",
        "\xF0\x9F\x86\x92",
        "\xF0\x9F\x86\x94",
        "\xF0\x9F\x86\x95",
        "\xF0\x9F\x86\x97",
        "\xF0\x9F\x86\x96",
        "\xF0\x9F\x86\x91",
        "\xF0\x9F\x86\x8E",
        "\xF0\x9F\x83\x8F",
        "\xF0\x9F\x80\x84",
        "\xF0\x9F\x85\xB0",
        "\xF0\x9F\x85\xB1",
        "\xF0\x9F\x85\xBF",
        "\xF0\x9F\x85\xBE",
        "\xF0\x9F\x86\x98",
        "\xF0\x9F\x86\x99",
        "\xF0\x9F\x88\xB5",
        "\xF0\x9F\x88\xB4",
        "\xF0\x9F\x88\xB6",
        "\xF0\x9F\x88\xB7",
        "\xF0\x9F\x88\xB9",
        "\xF0\x9F\x88\xB8",
        "\xF0\x9F\x88\xB3",
        "\xF0\x9F\x88\xB2",
        "\xF0\x9F\x88\x81",
        "\xF0\x9F\x86\x9A",
        "\xF0\x9F\x88\x82",
        "\xF0\x9F\x88\x9A",
        "\xF0\x9F\x88\xAF",
        "\xF0\x9F\x8C\x9C",
        "\xF0\x9F\x8C\x9D",
        "\xF0\x9F\x8D\x87",
        "\xF0\x9F\x8D\x86",
        "\xF0\x9F\x8D\x88",
        "\xF0\x9F\x8D\x89",
        "\xF0\x9F\x8D\x8B",
        "\xF0\x9F\x8D\x8A",
        "\xF0\x9F\x8D\x85",
        "\xF0\x9F\x8D\x84",
        "\xF0\x9F\x8C\xBF",
        "\xF0\x9F\x8C\xBE",
        "\xF0\x9F\x8D\x80",
        "\xF0\x9F\x8D\x81",
        "\xF0\x9F\x8D\x83",
        "\xF0\x9F\x8D\x82",
        "\xF0\x9F\x8D\x8C",
        "\xF0\x9F\x8D\x8D",
        "\xF0\x9F\x8D\x96",
        "\xF0\x9F\x8D\x95",
        "\xF0\x9F\x8D\x97",
        "\xF0\x9F\x8D\x98",
        "\xF0\x9F\x8D\x9A",
        "\xF0\x9F\x8D\x99",
        "\xF0\x9F\x8D\x94",
        "\xF0\x9F\x8D\x93",
        "\xF0\x9F\x8D\x8F",
        "\xF0\x9F\x8D\x8E",
        "\xF0\x9F\x8D\x90",
        "\xF0\x9F\x8D\x91",
        "\xF0\x9F\x8D\x92",
        "\xF0\x9F\x8C\xBD",
        "\xF0\x9F\x8C\xBC",
        "\xF0\x9F\x8C\xA8",
        "\xF0\x9F\x8C\xA7",
        "\xF0\x9F\x8C\xA9",
        "\xF0\x9F\x8C\xAA",
        "\xF0\x9F\x8C\xAC",
        "\xF0\x9F\x8C\xAB",
        "\xF0\x9F\x8C\xA6",
        "\xF0\x9F\x8C\xA5",
        "\xF0\x9F\x8C\x9F",
        "\xF0\x9F\x8C\x9E",
        "\xF0\x9F\x8C\xA0",
        "\xF0\x9F\x8C\xA1",
        "\xF0\x9F\x8C\xA4",
        "\xF0\x9F\x8C\xAD",
        "\xF0\x9F\x8C\xAE",
        "\xF0\x9F\x8C\xB7",
        "\xF0\x9F\x8C\xB6",
        "\xF0\x9F\x8C\xB8",
        "\xF0\x9F\x8C\xB9",
        "\xF0\x9F\x8C\xBB",
        "\xF0\x9F\x8C\xBA",
        "\xF0\x9F\x8C\xB5",
        "\xF0\x9F\x8C\xB4",
        "\xF0\x9F\x8C\xB0",
        "\xF0\x9F\x8C\xAF",
        "\xF0\x9F\x8C\xB1",
        "\xF0\x9F\x8C\xB2",
        "\xF0\x9F\x8C\xB3",
        "\xF0\x9F\x8F\x9C",
        "\xF0\x9F\x8E\x86",
        "\xF0\x9F\x92\x83",
        "\xF0\x9F\x92\x82",
        "\xF0\x9F\x92\x84",
        "\xF0\x9F\x92\x85",
        "\xF0\x9F\x92\x87",
        "\xF0\x9F\x92\x86",
        "\xF0\x9F\x92\x81",
        "\xF0\x9F\x92\x80",
        "\xF0\x9F\x91\xBB",
        "\xF0\x9F\x91\xBA",
        "\xF0\x9F\x91\xBC",
        "\xF0\x9F\x91\xBD",
        "\xF0\x9F\x91\xBF",
        "\xF0\x9F\x91\xBE",
        "\xF0\x9F\x92\x88",
        "\xF0\x9F\x92\x89",
        "\xF0\x9F\x92\x92",
        "\xF0\x9F\x92\x91",
        "\xF0\x9F\x92\x93",
        "\xF0\x9F\x92\x94",
        "\xF0\x9F\x92\x96",
        "\xF0\x9F\x92\x95",
        "\xF0\x9F\x92\x90",
        "\xF0\x9F\x92\x8F",
        "\xF0\x9F\x92\x8B",
        "\xF0\x9F\x92\x8A",
        "\xF0\x9F\x92\x8C",
        "\xF0\x9F\x92\x8D",
        "\xF0\x9F\x92\x8E",
        "\xF0\x9F\x91\xB9",
        "\xF0\x9F\x91\xB8",
        "\xF0\x9F\x91\xA4",
        "\xF0\x9F\x91\xA3",
        "\xF0\x9F\x91\xA5",
        "\xF0\x9F\x91\xA6",
        "\xF0\x9F\x91\xA8",
        "\xF0\x9F\x91\xA7",
        "\xF0\x9F\x91\xA2",
        "\xF0\x9F\x91\xA1",
        "\xF0\x9F\x91\x9C",
        "\xF0\x9F\x91\x9B",
        "\xF0\x9F\x91\x9D",
        "\xF0\x9F\x91\x9E",
        "\xF0\x9F\x91\xA0",
        "\xF0\x9F\x91\x9F",
        "\xF0\x9F\x91\xA9",
        "\xF0\x9F\x91\xAA",
        "\xF0\x9F\x91\xB3",
        "\xF0\x9F\x91\xB2",
        "\xF0\x9F\x91\xB4",
        "\xF0\x9F\x91\xB5",
        "\xF0\x9F\x91\xB7",
        "\xF0\x9F\x91\xB6",
        "\xF0\x9F\x91\xB1",
        "\xF0\x9F\x91\xB0",
        "\xF0\x9F\x91\xAC",
        "\xF0\x9F\x91\xAB",
        "\xF0\x9F\x91\xAD",
        "\xF0\x9F\x91\xAE",
        "\xF0\x9F\x91\xAF",
        "\xF0\x9F\x92\x98",
        "\xF0\x9F\x92\x99",
        "\xF0\x9F\x93\x81",
        "\xF0\x9F\x93\x80",
        "\xF0\x9F\x93\x82",
        "\xF0\x9F\x93\x83",
        "\xF0\x9F\x93\x85",
        "\xF0\x9F\x93\x84",
        "\xF0\x9F\x92\xBF",
        "\xF0\x9F\x92\xBE",
        "\xF0\x9F\x92\xB9",
        "\xF0\x9F\x92\xB8",
        "\xF0\x9F\x92\xBA",
        "\xF0\x9F\x92\xBB",
        "\xF0\x9F\x92\xBD",
        "\xF0\x9F\x92\xBC",
        "\xF0\x9F\x93\x86",
        "\xF0\x9F\x93\x87",
        "\xF0\x9F\x93\x90",
        "\xF0\x9F\x93\x8F",
        "\xF0\x9F\x93\x91",
        "\xF0\x9F\x93\x92",
        "\xF0\x9F\x93\x94",
        "\xF0\x9F\x93\x93",
        "\xF0\x9F\x93\x8E",
        "\xF0\x9F\x93\x8D",
        "\xF0\x9F\x93\x89",
        "\xF0\x9F\x93\x88",
        "\xF0\x9F\x93\x8A",
        "\xF0\x9F\x93\x8B",
        "\xF0\x9F\x93\x8C",
        "\xF0\x9F\x92\xB7",
        "\xF0\x9F\x92\xB6",
        "\xF0\x9F\x92\xA2",
        "\xF0\x9F\x92\xA1",
        "\xF0\x9F\x92\xA3",
        "\xF0\x9F\x92\xA4",
        "\xF0\x9F\x92\xA6",
        "\xF0\x9F\x92\xA5",
        "\xF0\x9F\x92\xA0",
        "\xF0\x9F\x92\x9F",
        "\xF0\x9F\x92\x9B",
        "\xF0\x9F\x92\x9A",
        "\xF0\x9F\x92\x9C",
        "\xF0\x9F\x92\x9D",
        "\xF0\x9F\x92\x9E",
        "\xF0\x9F\x92\xA7",
        "\xF0\x9F\x92\xA8",
        "\xF0\x9F\x92\xB1",
        "\xF0\x9F\x92\xB0",
        "\xF0\x9F\x92\xB2",
        "\xF0\x9F\x92\xB3",
        "\xF0\x9F\x92\xB5",
        "\xF0\x9F\x92\xB4",
        "\xF0\x9F\x92\xAF",
        "\xF0\x9F\x92\xAE",
        "\xF0\x9F\x92\xAA",
        "\xF0\x9F\x92\xA9",
        "\xF0\x9F\x8F\x9D",
        "\xF0\x9F\x92\xAC",
        "\xF0\x9F\x92\xAD",
        "\xF0\x9F\x91\x9A",
        "\xF0\x9F\x92\xAB",
        "\xF0\x9F\x90\x89",
        "\xF0\x9F\x90\x88",
        "\xF0\x9F\x90\x8A",
        "\xF0\x9F\x90\x8B",
        "\xF0\x9F\x90\x8D",
        "\xF0\x9F\x90\x8C",
        "\xF0\x9F\x90\x87",
        "\xF0\x9F\x90\x86",
        "\xF0\x9F\x90\x81",
        "\xF0\x9F\x90\x80",
        "\xF0\x9F\x90\x82",
        "\xF0\x9F\x90\x83",
        "\xF0\x9F\x90\x85",
        "\xF0\x9F\x90\x84",
        "\xF0\x9F\x90\x8E",
        "\xF0\x9F\x90\x8F",
        "\xF0\x9F\x90\x98",
        "\xF0\x9F\x90\x97",
        "\xF0\x9F\x90\x99",
        "\xF0\x9F\x90\x9A",
        "\xF0\x9F\x90\x9C",
        "\xF0\x9F\x90\x9B",
        "\xF0\x9F\x90\x96",
        "\xF0\x9F\x90\x95",
        "\xF0\x9F\x90\x91",
        "\xF0\x9F\x90\x90",
        "\xF0\x9F\x90\x92",
        "\xF0\x9F\x90\x93",
        "\xF0\x9F\x90\x94",
        "\xF0\x9F\x8F\xBF",
        "\xF0\x9F\x8F\xBE",
        "\xF0\x9F\x8F\xA6",
        "\xF0\x9F\x8F\xA5",
        "\xF0\x9F\x8F\xA7",
        "\xF0\x9F\x8F\xA8",
        "\xF0\x9F\x8F\xAA",
        "\xF0\x9F\x8F\xA9",
        "\xF0\x9F\x8F\xA4",
        "\xF0\x9F\x8F\xA3",
        "\xF0\x9F\x8F\x9E",
        "\xF0\x9F\x91\x99",
        "\xF0\x9F\x8F\x9F",
        "\xF0\x9F\x8F\xA0",
        "\xF0\x9F\x8F\xA2",
        "\xF0\x9F\x8F\xA1",
        "\xF0\x9F\x8F\xAB",
        "\xF0\x9F\x8F\xAC",
        "\xF0\x9F\x8F\xB9",
        "\xF0\x9F\x8F\xB8",
        "\xF0\x9F\x8F\xBA",
        "\xF0\x9F\x8F\xBB",
        "\xF0\x9F\x8F\xBD",
        "\xF0\x9F\x8F\xBC",
        "\xF0\x9F\x8F\xB7",
        "\xF0\x9F\x8F\xB4",
        "\xF0\x9F\x8F\xAE",
        "\xF0\x9F\x8F\xAD",
        "\xF0\x9F\x8F\xAF",
        "\xF0\x9F\x8F\xB0",
        "\xF0\x9F\x8F\xB3",
        "\xF0\x9F\x90\x9D",
        "\xF0\x9F\x8F\xB5",
        "\xF0\x9F\x91\x84",
        "\xF0\x9F\x91\x83",
        "\xF0\x9F\x91\x85",
        "\xF0\x9F\x91\x86",
        "\xF0\x9F\x91\x88",
        "\xF0\x9F\x91\x87",
        "\xF0\x9F\x91\x82",
        "\xF0\x9F\x91\x81",
        "\xF0\x9F\x90\xBD",
        "\xF0\x9F\x90\xBC",
        "\xF0\x9F\x90\xBE",
        "\xF0\x9F\x90\xBF",
        "\xF0\x9F\x91\x80",
        "\xF0\x9F\x91\x89",
        "\xF0\x9F\x91\x8A",
        "\xF0\x9F\x91\x94",
        "\xF0\x9F\x91\x93",
        "\xF0\x9F\x91\x95",
        "\xF0\x9F\x91\x96",
        "\xF0\x9F\x91\x98",
        "\xF0\x9F\x91\x97",
        "\xF0\x9F\x91\x92",
        "\xF0\x9F\x91\x91",
        "\xF0\x9F\x91\x8C",
        "\xF0\x9F\x91\x8B",
        "\xF0\x9F\x91\x8E",
        "\xF0\x9F\x91\x8F",
        "\xF0\x9F\x91\x90",
        "\xF0\x9F\x90\xBB",
        "\xF0\x9F\x91\x8D",
        "\xF0\x9F\x90\xA7",
        "\xF0\x9F\x90\xA6",
        "\xF0\x9F\x90\xA8",
        "\xF0\x9F\x90\xA9",
        "\xF0\x9F\x90\xAB",
        "\xF0\x9F\x90\xAA",
        "\xF0\x9F\x90\xA4",
        "\xF0\x9F\x90\xA3",
        "\xF0\x9F\x90\xBA",
        "\xF0\x9F\x90\x9E",
        "\xF0\x9F\x90\x9F",
        "\xF0\x9F\x90\xA0",
        "\xF0\x9F\x90\xA2",
        "\xF0\x9F\x90\xA1",
        "\xF0\x9F\x90\xAC",
        "\xF0\x9F\x90\xA5",
        "\xF0\x9F\x90\xB6",
        "\xF0\x9F\x90\xB5",
        "\xF0\x9F\x90\xB7",
        "\xF0\x9F\x90\xAD",
        "\xF0\x9F\x90\xB8",
        "\xF0\x9F\x90\xB3",
        "\xF0\x9F\x90\xB4",
        "\xF0\x9F\x90\xB9",
        "\xF0\x9F\x90\xAF",
        "\xF0\x9F\x90\xAE",
        "\xF0\x9F\x90\xB0",
        "\xF0\x9F\x90\xB1",
        "\xF0\x9F\x90\xB2",
        "\xE2\x98\xA2",
        "\xE2\x98\xA6",
        "\xE2\x98\xA3",
        "\xE2\x98\xAA",
        "\xE2\x98\xAF",
        "\xE2\x98\xB8",
        "\xE2\x98\xA0",
        "\xE2\x98\xB9",
        "\xE2\x98\xAE",
        "\xE2\x98\x8E",
        "\xE2\x98\x82",
        "\xE2\x98\xBA",
        "\xE2\x98\x83",
        "\xE2\x98\x84",
        "\xE2\x98\x91",
        "\xE2\x98\x98",
        "\xE2\x98\x95",
        "\xE2\x98\x94",
        "\xE2\x98\x9D",
        "\xE2\x99\xA5",
        "\xE2\x99\xA6",
        "\xE2\x99\xA3",
        "\xE2\x99\xA0",
        "\xE2\x99\x93",
        "\xE2\x99\xA8",
        "\xE2\x99\xBB",
        "\xE2\x9A\x92",
        "\xE2\x98\x81",
        "\xE2\x99\xBF",
        "\xE2\x99\x92",
        "\xE2\x99\x91",
        "\xE2\x99\x8B",
        "\xE2\x99\x8A",
        "\xE2\x99\x89",
        "\xE2\x99\x8C",
        "\xE2\x99\x8D",
        "\xE2\x99\x90",
        "\xE2\x99\x8F",
        "\xE2\x99\x8E",
        "\xE2\x99\x88",
        "\xE2\x86\xA9",
        "\xE2\x8C\x9B",
        "\xE2\x8C\x9A",
        "\xE2\x86\xAA",
        "\xE2\x80\xBC",
        "\xE2\x8C\xA8",
        "\xE2\x8F\x8F",
        "\xE2\x8F\xAB",
        "\xE2\x8F\xAA",
        "\xE2\x8F\xA9",
        "\xE2\x86\x99",
        "\xE2\x86\x98",
        "\xE2\x84\xA2",
        "\xE2\x81\x89",
        "\xE2\x9A\x93",
        "\xE2\x84\xB9",
        "\xE2\x86\x94",
        "\xE2\x86\x97",
        "\xE2\x86\x96",
        "\xE2\x86\x95",
        "\xE2\x8F\xAC",
        "\xE2\x8F\xAD",
        "\xE2\x96\xB6",
        "\xE2\x96\xAB",
        "\xE2\x96\xAA",
        "\xE2\x97\x80",
        "\xE2\x97\xBB",
        "\xE2\x97\xBE",
        "\xE2\x97\xBD",
        "\xE2\x97\xBC",
        "\xE2\x93\x82",
        "\xE2\x8F\xBA",
        "\xE2\x8F\xB0",
        "\xE2\x8F\xAF",
        "\xE2\x8F\xAE",
        "\xE2\x8F\xB1",
        "\xE2\x8F\xB2",
        "\xE2\x8F\xB9",
        "\xE2\x8F\xB8",
        "\xE2\x8F\xB3",
        "\xE2\x98\x80",
        "\xE2\x9D\xA3",
        "\xE2\x9D\x8C",
        "\xE2\x9D\x87",
        "\xE2\x9D\x84",
        "\xE2\x9C\xB4",
        "\xE2\x9D\x8E",
        "\xE2\x9D\x93",
        "\xE2\x9D\x97",
        "\xE2\x9D\x95",
        "\xE2\x9D\x94",
        "\xE2\x9C\xB3",
        "\xE2\x9C\xA8",
        "\xE2\x9C\x8F",
        "\xE2\x9C\x8D",
        "\xE2\x9C\x8C",
        "\xE2\x9C\x92",
        "\xE2\x9C\x94",
        "\xE2\x9C\xA1",
        "\xE2\x9C\x9D",
        "\xE2\x9C\x96",
        "\xE2\x9D\xA4",
        "\xE2\x9E\x95",
        "\xE2\xAD\x90",
        "\xE2\xAC\x9C",
        "\xE2\xAC\x9B",
        "\xE2\xAD\x95",
        "\xE3\x80\xB0",
        "\xE3\x8A\x99",
        "\xE3\x8A\x97",
        "\xE3\x80\xBD",
        "\xE2\xAC\x87",
        "\xE2\xAC\x86",
        "\xE2\x9E\xA1",
        "\xE2\x9E\x97",
        "\xE2\x9E\x96",
        "\xE2\x9E\xB0",
        "\xE2\x9E\xBF",
        "\xE2\xAC\x85",
        "\xE2\xA4\xB5",
        "\xE2\xA4\xB4",
        "\xE2\x9A\x94",
        "\xE2\x9C\x8B",
        "\xE2\x9A\xBE",
        "\xE2\x9A\xBD",
        "\xE2\x9A\xB1",
        "\xE2\x9B\x84",
        "\xE2\x9B\x85",
        "\xE2\x9B\x8F",
        "\xE2\x9B\x8E",
        "\xE2\x9B\x88",
        "\xE2\x9A\xB0",
        "\xE2\x9A\xAB",
        "\xE2\x9A\x99",
        "\xE2\x9A\x97",
        "\xE2\x9A\x96",
        "\xE2\x9A\x9B",
        "\xE2\x9A\x9C",
        "\xE2\x9A\xAA",
        "\xE2\x9A\xA1",
        "\xE2\x9A\xA0",
        "\xE2\x9C\x8A",
        "\xE2\x9B\x91",
        "\xE2\x9B\xBA",
        "\xE2\x9B\xB9",
        "\xE2\x9B\xB8",
        "\xE2\x9B\xBD",
        "\xE2\x9C\x82",
        "\xE2\x9C\x89",
        "\xE2\x9C\x88",
        "\xE2\x9C\x85",
        "\xE2\x9B\x93",
        "\xE2\x9B\xB7",
        "\xE2\x9B\xAA",
        "\xE2\x9B\xA9",
        "\xE2\x9B\x94",
        "\xE2\x9B\xB5",
        "\xE2\x9B\xB0",
        "\xE2\x9B\xB4",
        "\xE2\x9B\xB1",
        "\xE2\x9B\xB3",
        "\xE2\x9B\xB2",
        "\xC2\xAE",
        "\xC2\xA9",
    	];
    	
		return str_replace($table, '', $text);
    }
}
